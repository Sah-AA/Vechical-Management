...
import { Manager, managersApi, ManagerSalaryHistory } from "../../../lib/api/managers";
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--foreground-subtle)", marginTop: 3 }}>
              {manager.name} · {month ?? "Current month"}
            </p>
          </div>
...
export default function ManagerDetailPage() {
  ...
  useEffect(() => {
    ...
    managersApi
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(getErrorMessage(err, "Failed to load manager details"));
      })
      .finally(() => {
        if (!cancelled) {
          setLoading(false);
  ...
  const salaryHistory = useMemo<ManagerSalaryHistory[]>(
    () => (manager?.salaryHistory ?? []).slice().sort((a, b) => b.month.localeCompare(a.month)),
    [manager],
  );

  async function refreshManager() {
    const updated = await managersApi.getOne(id);
    setManager(updated);
  }
  ...
  async function submitPaySalary() {
    ...
    setPayError(null);
    try {
      await managersApi.paySalary(id, {
  ...
  if (loading) {
    return <LoadingUI fullPage label="Loading manager details..." />;
  }
    ...
    return (
      <div style={{ minHeight:"100vh",background:"var(--background)",display:"flex",alignItems:"center",justifyContent:"center" }}>
        <div style={{ textAlign:"center" }}>
          <p style={{ fontSize:16,color:"var(--foreground-muted)" }}>{error ?? "Manager not found."}</p>
          <button className="btn btn-primary" style={{ marginTop:16 }} onClick={() => router.push("/managers")}>
            <ArrowLeft size={14}/> Back to Managers
          </button>
        </div>
      </div>
    );
  ...
  const avatarColor = managerAvatarColors[manager.id % managerAvatarColors.length];
  ...
  const tenureMonths = Math.floor((Date.now() - new Date(manager.joiningDate).getTime()) / (1000 * 60 * 60 * 24 * 30));

  return (
    <div style={{ minHeight:"100vh",background:"var(--background)" }}>
      <Navbar title="Manager Details" subtitle={`Full profile and salary report for ${manager.name}`} />
      ...
      <div style={{ padding:"28px 28px 48px",display:"flex",flexDirection:"column",gap:20 }}>
          ...
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:16 }}>
              ...
              <div>
                ...
                <div style={{ display:"flex",flexWrap:"wrap",gap:12 }}>
                  <span style={{ display:"flex",alignItems:"center",gap:5,fontSize:13,color:"var(--foreground-muted)" }}>
                    <Phone size={13}/>{manager.mobile}
                  </span>
                  <span style={{ display:"flex",alignItems:"center",gap:5,fontSize:13,color:"var(--foreground-muted)" }}>
                    <CalendarDays size={13}/>Joined {new Date(manager.joiningDate).toLocaleDateString()} · {tenureMonths}m tenure
                  </span>
                </div>
                <div style={{ marginTop:8 }}>
                  <span className={`badge ${manager.status === "Active" ? "badge-active" : "badge-free"}`} style={{ fontSize:12.5 }}>
                    {manager.status === "Active" ? <CheckCircle2 size={10}/> : <AlertCircle size={10}/>}
                    {manager.status}
                  </span>
                </div>
              </div>
            ...
            <div style={{ textAlign:"right" }}>
              ...
              <p style={{ fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:28,color:"var(--amber-dark)",letterSpacing:"-0.02em" }}>
                ₹{manager.monthlySalary.toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16 }}>
          {[
            { label:"Monthly Salary",    value:`₹${manager.monthlySalary.toLocaleString()}`, icon:IndianRupee, accent:"#F59E0B", bg:"rgba(245,158,11,0.08)" },
            { label:"Advance Balance",   value:`₹${manager.advanceBalance.toLocaleString()}`,icon:Wallet,      accent:"#EA580C", bg:"rgba(234,88,12,0.08)"  },
          ...
          <div className="card animate-fade-up stagger-3" style={{ padding:"24px 26px" }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
              {[
                ...
                { label:"Advance Balance",   value:`₹${manager.advanceBalance.toLocaleString()}`,color:"#EA580C", icon: TrendingDown },
